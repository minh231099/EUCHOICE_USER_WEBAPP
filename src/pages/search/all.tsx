...
const SearchPage = (props: SearchPagePropsInf) => {
    const [open, setOpen] = useState(false);
    const [keyS, setKeyS] = useState<any>(undefined);
    const router = useRouter();

    useEffect(() => {
        setKeyS(router.query.key)
    }, [router.query.key])
            ...
            <div className='div-grid'>
                ...
                <div className='div-pagination-prod'>
                    <Pagination
                        ...
                        pageSize={20}
                        onChange={(e) => { router.push({ pathname: `/search/all`, query: { key: keyS, page: e } }) }}
                    />
                </div>
            </div>
        </div>
...
export const getServerSideProps = async (props: any) => {
    const api = API_URLS.SEARCH.searchProduct(`?key=${props.query.key}&page=${props.query.page}`);
